...
- **Members** are the specialists you keep across projects, on the **Team** page: a named implementer, reviewer or QA with an engine, instructions of its own, an avatar and what it has learned. A project's team can take a member in place of the template's role; the member brings its name, engine and instructions, after the template's. A learning is something the member should keep doing everywhere, such as a correction you made to its work. You add learnings on the Team page, or the assistant records one when you say so. They are added to the member's instructions when each of its tasks starts, in any project, so one learned mid-task reaches its next task.
