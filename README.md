...
Demo mode uses a temporary state that is removed on exit, disables integrations and never runs a model.
...
- **Tasks** are the outcomes you ask for. Each runs through a loop: the writer drafts in the project's private workspace; each reviewer reads its own copy against the brief and returns pass, revise or a question; findings go back to the writer until the reviewers pass or the round limit is reached.
...
The home screen shows decisions first, then projects with a one-line status. A project page has its brief, its team, a box to ask for something, its tasks, the latest draft rendered, and — one click deeper — every round's drafts and reviews. Memory is visible and editable. **Settings** holds the assistant's identity, model, conversation options and limits.

To attach files to a message, drop them onto the conversation box or paste them into it. They wait above the box, where you can remove them, until you send. Only UTF-8 text files can be attached: any `text/*` type, JSON, XML, YAML, TOML, and common text and source extensions such as `.md`, `.csv`, `.log`, `.go` or `.py`. Each file is sent inside your message as a labelled block. The message and its attachments together can be at most 24,000 bytes, the daemon's message limit. Images, PDFs and other binary files are refused with a reason; they need a transport the daemon does not have yet. If a clipboard holds text, pasting inserts the text as usual. A message the daemon refuses can be restored to the box with its attachments.
