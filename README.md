...
The home screen shows decisions first, then projects with a one-line status. A project page has its brief, its team, a box to ask for something, its tasks, the latest draft rendered, and — one click deeper — every round's drafts and reviews. Memory is visible and editable. Once a reply has landed and nothing is waiting, the empty message box may suggest your next message, written by the small model on the assistant's own login (Luna for Codex, Haiku for Claude); Tab takes it as a draft to edit, and typing dismisses it. It never sends anything or runs a tool. **Settings** holds the assistant's identity, model, conversation options and limits.
