...
func (s *Service) CreateDecision(ctx context.Context, in DecisionInput) (Decision, error) {
	...
	out := Decision{ID: uid(), ProjectID: in.ProjectID, Title: in.Title, Context: in.Context, Recommendation: in.Recommendation, Choices: in.Choices, Status: "open", CreatedAt: s.now().UTC()}
