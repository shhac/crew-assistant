...
func TestToolSchemaContainsOnlyCoordinationSurface(t *testing.T) {
	required := map[string]bool{"read_state": true, "create_project": true, "update_project": true, "ask_decision": true, "remember_preference": true, "report_status": true, "list_connections": true, "query_connection": true}
